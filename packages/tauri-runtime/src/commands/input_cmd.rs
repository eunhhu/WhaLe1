...
#[tauri::command]
pub fn input_simulate_key_press(window: WebviewWindow, key: String) -> Result<(), String> {
    ensure_privileged_window(&window)?;
...
#[tauri::command]
pub fn input_simulate_key_down(window: WebviewWindow, key: String) -> Result<(), String> {
    ensure_privileged_window(&window)?;
    let rdev_key = string_to_key(&key).ok_or(format!("Unknown key: {}", key))?;
    simulate(&EventType::KeyPress(rdev_key)).map_err(|e| format!("{:?}", e))
}
...
#[tauri::command]
pub fn input_simulate_key_up(window: WebviewWindow, key: String) -> Result<(), String> {
    ensure_privileged_window(&window)?;
    let rdev_key = string_to_key(&key).ok_or(format!("Unknown key: {}", key))?;
    simulate(&EventType::KeyRelease(rdev_key)).map_err(|e| format!("{:?}", e))
}
...
#[tauri::command]
pub fn input_simulate_mouse_click(window: WebviewWindow, x: f64, y: f64) -> Result<(), String> {
    ensure_privileged_window(&window)?;
...
#[tauri::command]
pub fn input_simulate_mouse_move(window: WebviewWindow, x: f64, y: f64) -> Result<(), String> {
    ensure_privileged_window(&window)?;
    simulate(&EventType::MouseMove { x, y }).map_err(|e| format!("{:?}", e))
}
